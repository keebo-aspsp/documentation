...
## Base URL
The base URL for all AIS APIs is: `https://rs1.ob.keebo.io/open-banking/v3.1/pisp/**`

## Supported Payment Types
The KEEBO API currently only supports:
- Domestic Payments
- Domestic Scheduled Payments
- Domestic Standing Orders

The KEEBO API __does not__ support:
...
- There is no MAX `InstructedAmount/Amount` mandated by KEEBO API.
- 50,000 GBP is the default maximum when opening a KEEBO account, but thresholds can be managed by the customer. KEEBO suggest PISP notify the PSU that the same limits apply as in their KEEBO app. It is possible from time to time that `domestic-payment-consents` is authorised, but the payment initiation fails due to account limits.
...
- Notify PSU that only payments to existing recipients are permitted and in the event this condition is not met KEEBO will not authorise the consent and display error modal to user
- Integrate with KEEBO beneficiaries API and can confirm that the recipient already exists before sending the consent to allow for a better user experience
...
However, this field is not mandatory so we suggest PISP simply not include this field and KEEBO will stage consent as a faster payment.
...
- `Initiation/Frequency` supported by KEEBO  are:
  - `IntrvlMnthDay:01:xx`
  - `IntrvlWkDay:01:xx`
...
- last two digits after IntrvlMnthDay are not considered or validated by KEEBO  (payment will be made as per the day in FirstPaymentDateTime)
...
- last two digits after IntrvlMnthWkDay are not taken in to account by KEEBO (payment will be made as per the day in FirstPaymentDateTime)